...
sealed interface DownloaderAction {
    ...
    data class DownloadSeason(val episodes: List<FindroidEpisode>, val storageIndex: Int = 0) : DownloaderAction

    data class DeleteDownload(val item: FindroidItem) : DownloaderAction

    data class DeleteDownloadedSeason(val episodes: List<FindroidEpisode>) : DownloaderAction

    data class CancelDownload(val item: FindroidItem) : DownloaderAction

    data object CancelDownloadSeason: DownloaderAction
}
